...
void loop() {
  ...
  if (sayac==100){
    ...
    lcd.write(1);
    delay(10);
    if(Firebase.getJSON(datarfid, "/ogenciler")){
      paraData = datarfid.jsonString();
      d.jload(paraData);
      sifre=d["password"];
      
     }
    else{
      Serial.println(datarfid.errorReason());
    }
    sayac=0;
  ...
  cardnumber="bos";
    ...
    if(fonksiyon==0){
      ...
      mevcutpara=d[cardnumber].toFloat();
      Serial.println(cardnumber);
      if(mevcutpara>=buy.toFloat()){
        
        float kalan = mevcutpara-buy.toFloat();
        Firebase.setString(datarfid, "ogenciler/"+cardnumber, String(kalan));
      ...
      Firebase.setString(datarfid, "ogenciler/"+cardnumber, String(newPara));
      lcd.setCursor(0, 0);
